...
for FILE in ${MARKDOWN_FILES}; do
    WORDS=$(hunspell -d "${DICTIONARY}" -p documentation/dictionary/lsl-processor.dic -l "${FILE}" | sort | uniq)
...
for FILE in ${TEX_FILES}; do
    WORDS=$(hunspell -d "${DICTIONARY}" -p documentation/dictionary/lsl-processor.dic -l -t "${FILE}")
